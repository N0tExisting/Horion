...
//	if (strcmp(currentEntity->getNameTag()->getText(), "qrobi1") == 0) return;
...
void CrystalAura2::onTick(C_GameMode* gm) {
	...
	if (g_Data.getLocalPlayer()->getSelectedItemId() == 466) return;
		...
		if ((crystalDelay >= this->delay) && !(targetList7.empty())) {
			...
			if (!checkSurrounded2(targetList7[0])) {
				...
				if (!gucciPositions.empty())
					if (g_Data.getLocalPlayer()->getSelectedItemId() == 426) {
						placeArr.clear();
	...
	g_Data.forEachEntity([](C_Entity* ent, bool b) {
		...
		if (id == 71 && g_Data.getLocalPlayer()->getPos()->dist(*ent->getPos()) <= range) {
			...
			hasPlaced = false;
			hitArr.push_back(new vec3_t(ent->getAABB()->centerPoint()));
